...
import { NgxStepperSettings, StepperOrientation } from '../../services/stepper-settings/stepper-settings.service';
...
@Component({
  ...
  providers: [{ provide: NgxStepper, useExisting: StepperComponent }, NgxStepperSettings],
  animations: [
...
export class StepperComponent extends NgxStepper implements AfterContentInit, OnDestroy {
  @ContentChildren(NgxStep) private readonly _steps?: QueryList<NgxStep>;
  ...
  public constructor(public readonly stepperSettings: NgxStepperSettings, private readonly elementRef: ElementRef) {
    super();
  ...
  public ngAfterContentInit(): void {
    this._steps!.changes.pipe(
      ...
      map(steps => steps.toArray() as NgxStep[]),
      tap(steps => {
