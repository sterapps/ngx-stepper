...
export class StepperHeaderComponent implements OnDestroy {
  ...
  private readonly isFirstStep$ = this.step.oneBasedIndex$.pipe(
    map((index) => index === 1)
  );
  ...
  public readonly topStepConnectionLineVisible$ = combineLatest([
    this.isFirstStep$,
    this.stepperSettings.hasStepConnectionLine$,
  ]).pipe(
    map(
      ([isFirstStep, hasStepConnectionLine]) =>
        !isFirstStep && hasStepConnectionLine
    )
  );
  ...
  public readonly bottomStepConnectionLineVisible$ = combineLatest([
    ...
    map(
      ([isLastStep, hasStepConnectionLine]) =>
        !isLastStep && hasStepConnectionLine
    )
  );
  ...
  public constructor(
    ...
    this.headerNavigationEnabledHandler$
  ...
  private readonly headerNavigationEnabledHandler$ =
    this.stepperSettings.headerNavigationEnabled$.pipe(
      distinctUntilChanged(),
      tap((enabled) => {
        enabled
